...
else
{
	...
	$result=mysqli_query($conn,"select * from study_material WHERE `studyMatId`='$studyMatId'");
	...
	{	
		...
		$studyMatId=$row['studyMatId'];
		$studyName=$row['studyMatName'];
		$studyDescription=$row['studyMatDesc'];
		$gradeName=$row['grade'];
		$subjectName=$row['subject'];
		$unitsName=$row['topicOrUnit'];
	...
	if(isset($_POST['submit']))//for update study material
	{
		$studyMatId=$_POST['studyMatId'];
		...
		$result1=mysqli_query($conn,"UPDATE `study_material` set `studyMatName`='$studyName', `studyMatDesc`='$studyDescription', `gradeId`='$gradeId', `grade`='$gradeName', 
		`subjectId`='$subjectId', `subject`='$subjectName', `topicId`='$unitsId', `topicOrUnit`='$unitsName', `chapterId`='$chapterId', `chapter`='$chapterName', `addedOn`='$addedOn', `addedBy`='$addedBy', `status`='$status' WHERE `studyMatId`='$studyMatId'");
		if(mysqli_affected_rows($conn) > 0)
		{
			$dir="../../studymaterial/XII/math/".$chapterName."/studyMatId_".$studyMatId;
			
			chdir ($dir);
			$path1 = "$root"."/backend/yofundowebapp/studymaterial/XII/math/".$chapterName."/studyMatId_".$studyMatId."/";
			$upload_url1 = "http://"."34.229.183.41/backend/yofundowebapp/studymaterial/XII/math/".$chapterName."/studyMatId_".$studyMatId."/";
			...
			if (isset($_POST['checkbox_pdf']))
			{
				...
				if(unlink('doc.'.$exten))
				{
					$result6=mysqli_query($conn,"UPDATE `study_material` SET `fileSize`='', `fileName`='', `fileType`='$documentType' WHERE `studyMatId`='$studyMatId'");
				}
				else
				{
					echo "<script type='text/javascript'>alert('Cannot remove!')</script>";
				}
			}	
			
			if (isset($_POST['checkbox_video']))
			{
				$dir="../../studymaterial/XII/math/".$chapterName."/studyMatId_".$studyMatId;
				chdir ($dir);
				$exten = pathinfo($fileName, PATHINFO_EXTENSION);
				if(unlink('video.'.$exten))
				{
					$result6=mysqli_query($conn,"UPDATE `study_material` SET `fileSize`='', `fileName`='', `fileType`='$documentType' WHERE `studyMatId`='$studyMatId'");
				}
				else
		...
		{
			echo "<script type='text/javascript'>alert('Cannot Update!')</script>";
		}
	}
?>
<!DOCTYPE html>
...
{
	var documentType = <?php echo "$documentType"; ?>;
      ...
      <div class="row">
		...
		<div class="box-header with-border">
          ...
          <div class="nav-tabs-custom">
			...
			<!-- hidden -->
			<input type="hidden" class="form-control" name="studyMatId" id="studyMatId" value="<?php echo $studyMatId; ?>">
			...
			<div class="tab-content" style="padding: 40px;">
			  ...
			  <div class="tab-pane" id="studyMaterial">
				...
				<div class="form-group">
				  ...
				  <div class="col-sm-10" required>
					<select class="form-control select2" name="documentType" id="documentType" style="width: 100%;">
					  <option selected="selected" disabled>--Select--</option>
					  <option value="1" <?php echo (($status=="1")?'selected="selected"':"");?>>Pdf</option>
					  <option value="2" <?php echo (($status=="2")?'selected="selected"':"");?>>Video</option>
					</select>			  
				  </div>
                </div>
				...
				<div class="form-group" id="document_pdf" style="display:none;">
				  ...
				  <input type='checkbox' class='flat-red' value='1' name='checkbox_pdf' id='checkbox_pdf' style="display:none;"> Remove
	<!--			  
				  <embed src="<?php //echo $fileName; ?>" type="application/pdf" id="pdf_view" width="250px" height="250px" style="display:none;"/>
-->				  
				  <iframe src="<?php echo $fileName; ?>" style="width:250px; height:250px;" frameborder="0" id="pdf_view" style="display:none;"></iframe>
				...
				<div class="form-group" id="document_video" style="display:none;">
				  ...
				  <input type='checkbox' class='flat-red' value='1' name='checkbox_video' id='checkbox_video' style="display:none;"> Remove
				...
				<input type='submit' id="submit" name='submit' style="display:none;" class="btn btn-primary" value='Update Study Material'>
				</div>
			  </div>
